...
function Update-PSModulePathForHostedAgent {
    ...
    try {
        if ($targetAzurePs) {
            ...
            $hostedAgentAzModulePath = Get-LatestModule -patternToMatch "^az_[0-9]+\.[0-9]+\.[0-9]+$" `
                                                        -patternToExtract "[0-9]+\.[0-9]+\.[0-9]+$"
        }
...
function Update-PSModulePathForHostedAgentLinux {
    ...
    try {
        if ($targetAzurePs) {
            ...
            $hostedAgentAzModulePath = Get-LatestModuleLinux -patternToMatch "^az_[0-9]+\.[0-9]+\.[0-9]+$" `
                                                             -patternToExtract "[0-9]+\.[0-9]+\.[0-9]+$"
        }
        $env:PSModulePath = $hostedAgentAzModulePath + ":" + $env:PSModulePath
        $env:PSModulePath = $env:PSModulePath.TrimStart(':')
...
function Get-LatestModule {
    ...
    $resultFolder = ""
    $regexToMatch = New-Object -TypeName System.Text.RegularExpressions.Regex `
                               -ArgumentList $patternToMatch

    $regexToExtract = New-Object -TypeName System.Text.RegularExpressions.Regex `
                                 -ArgumentList $patternToExtract
    $maxVersion = [version] "0.0.0"

    try {
        $moduleFolders = Get-ChildItem -Directory `
                                       -Path $($env:SystemDrive + "\Modules") `
                                       | Where-Object { $regexToMatch.IsMatch($_.Name) }
